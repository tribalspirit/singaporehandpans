...
- Static routes: /, /about, /workshops, /events, /gallery, /contacts, /shop (feature-toggled).
- Academy route exists (placeholder) and will host the Handpan Memorization Widget: /academy.
- Shop route (/shop) integrates with Shopify Storefront API; feature-toggled via PUBLIC_ENABLE_SHOP.
...
## Shop (Shopify Integration) — Implementation Plan

Goal:
Integrate a Shop page that displays products from a Shopify store, allowing users to browse and filter products before being redirected to Shopify for checkout.

Core technical choices:

- Data source: Shopify Storefront GraphQL API (build-time fetch).
- UI: React island for client-side filtering; page shell is SSG.
- Feature toggle: PUBLIC_ENABLE_SHOP env var controls visibility.
- No cart/checkout: "Buy" links redirect to Shopify product pages.

Shop architecture (modules):

- src/lib/shopifyClient.ts: Shopify API client with fetchAllProducts() and isShopEnabled().
- src/components/ShopProductList.tsx: React island with search, filters, and product grid.
- src/components/ProductCard.tsx: Individual product card component.
- src/pages/shop.astro: Shop page route with feature toggle guard.

Data model:

- ShopProduct type: id, title, handle, productType, description, availableForSale, priceMin, priceMax, image, tags, shopUrl.
- Products normalized from Shopify GraphQL response.

Filtering approach:

- Client-side filtering for ~50-100 products.
- Search: title, description, tags (case-insensitive).
- Category: productType from Shopify.
- Price buckets: Under S$500, S$500-S$1,500, Above S$1,500.
- Availability: In Stock / Out of Stock based on availableForSale.

Feature toggle:

- Header: Shop link conditionally included based on isShopEnabled().
- Shop page: redirects to /404 when disabled.

Environment variables:

- PUBLIC_ENABLE_SHOP: Enable/disable shop feature (default: false).
- PUBLIC_SHOPIFY_STORE_DOMAIN: Shopify store domain.
- PUBLIC_SHOPIFY_STOREFRONT_TOKEN: Storefront API access token.
