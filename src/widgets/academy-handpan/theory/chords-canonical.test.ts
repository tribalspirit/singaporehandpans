...
describe('chords canonical naming', () => {
    ...
    it('should prefer Am7 over C6 for A-C-E-G pitch class set', () => {
      ...
      const c6Chord = chords.find((c) => {
        ...
        return (
          ...
          pcs.has('G') &&
          pcs.size === 4 &&
          c.displayName.includes('C6')
        );
      });

      if (c6Chord && am7Chord) {
        expect(c6Chord.pitchClasses.sort().join(',')).toBe(
          am7Chord.pitchClasses.sort().join(',')
        );
      }
    });
  });
    ...
    it('should not have duplicate displayNames with different pitch class sets', () => {
      ...
      for (const [displayName, pcsetKeys] of chordsByDisplayName.entries()) {
        if (pcsetKeys.size > 1) {
          ...
          const isSimpleTriad =
            baseName.length === 1 &&
            !displayName.includes('7') &&
            !displayName.includes('9') &&
            !displayName.includes('sus') &&
            !displayName.includes('add') &&
            !displayName.includes('m') &&
            !displayName.includes('°') &&
            !displayName.includes('+') &&
            !displayName.includes('6');
          ...
          if (isSimpleTriad) {
            ...
            if (!allAreTriads) {
              throw new Error(
                `Duplicate base name "${displayName}" found with different pcsets (some non-triads): ${pcsets.join('; ')}`
              );
            }
          }
        }
  ...
  describe('Bb presence in D-Kurd', () => {
    it('should include Bb chords when Bb is in scale', () => {
      ...
      const bbChords = chords.filter(
        (c) => c.rootPc === 'Bb' || c.rootPc === 'A#'
      );

      const hasBbPitchClass = availableNotes.some(
        (n) =>
          normalizeToPitchClass(n) === 'Bb' || normalizeToPitchClass(n) === 'A#'
      );

      if (hasBbPitchClass) {
        const bbTriad = chords.find(
          (c) =>
            (c.category === 'basic' &&
              (c.rootPc === 'Bb' || c.rootPc === 'A#') &&
              c.pitchClasses.includes('Bb')) ||
            c.pitchClasses.includes('A#')
        );
